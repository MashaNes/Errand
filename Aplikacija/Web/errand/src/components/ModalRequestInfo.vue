<template>
  <transition name="modal">
    <div class="modal-mask-report">
      <div class="modal-wrapper">
        <div class="modal-container">

          <Spinner v-if="!computedRequest" />

          <div class="modal-header" v-if="computedRequest">
            <h3 name="header" v-if="isSerbian">
              Detalji zahteva
            </h3>
            <h3 name="header" v-else>
              Request details
            </h3>
          </div>

          <div class="modal-body" v-if="computedRequest">
            <slot name="body">

                <div class="info-wrap">
                    <span v-if="isSerbian" class="info-span"> 
                        <span v-text="isSerbian ? 'Naziv: ' : 'Name: '" class="info-title"></span>
                        <strong> {{computedRequest.name}} </strong> 
                    </span>

                    <span class="info-span"> 
                        <span v-text="isSerbian ? 'Status završetka: ' : 'Ending status: '" class="info-title"> </span>
                        <strong :class="spanColor"> {{endingStatus}} </strong>  
                        <img src="@/assets/finished.svg" v-if="computedRequest.status == 2" class="info-pic-status" />
                        <img src="@/assets/failed.svg" v-if="computedRequest.status == 3" class="info-pic-status" />
                    </span>  
                    
                    <span v-if="isSerbian" class="info-span"> 
                        <span v-text="isSerbian ? 'Uloga: ' : 'Role: '" class="info-title"></span> 
                        <strong> {{role}} </strong> 
                    </span>

                    <span v-if="isSerbian" class="info-span"> 
                        <span v-text="isSerbian ? 'Zahtevane slike: ' : 'Pictures required: '" class="info-title"></span> 
                        <strong v-text="computedRequest.picture_required ? 'Da' : 'Ne'"></strong>
                        <img src="@/assets/photos.svg" class="info-pic" v-if="computedRequest.picture_required" />
                    </span>

                    <span v-if="isSerbian && computedRequest.picture_required" class="info-span"> 
                        <span v-text="isSerbian ? 'Priložene sve zahtevane slike: ' : 'All required pictures taken: '" class="info-title"></span>  
                        <strong v-text="picturesTaken ? 'Da' : 'Ne'"></strong> 
                        <img src="@/assets/happy.png" v-if="picturesTaken" class="info-pic" />
                        <img src="@/assets/sad.png" v-else class="info-pic" />
                    </span>
                </div>
            </slot>
          </div>

          <div class="modal-footer" v-if="computedRequest">
            <slot name="footer">
              <button 
                type="button" 
                class="btn btn-primary" 
                @click="$emit('showMore', computedRequest)"
                v-text="isSerbian ? 'Prikaži više' : 'Show more'"
                v-if="isParticipant"
              >
              </button>
              <button 
                type="button" 
                class="btn btn-secondary" 
                @click="$emit('close')"
                v-text="isSerbian ? 'Zatvori' : 'Close'"
              >
              </button>
            </slot>
          </div>
        </div>
      </div>
    </div>
  </transition>
  
</template>

<script>

import 'vue-slider-component/theme/default.css'
import Spinner from "@/components/Spinner"

export default {
  components: {
      Spinner
  },
  props: {
    insignificantUser: {
      type: Number,
      required: true
    },
    request: {
      type: Number, 
      required: true
    }
  },
  data() {
    return {
    }
  },
  computed: {
    isSerbian() {
      return this.$store.state.isSerbian
    },
    fullUserName() {
      return this.userToRate.first_name + " " +this.userToRate.last_name
    },
    computedRequest() {
        return this.$store.state.specificRequest
    },
    isRunner() {
        if(!this.computedRequest)
            return false
        else
            return this.computedRequest.created_by.id == this.insignificantUser
    },
    endingStatus() {
        let retValue = ""
        switch(this.computedRequest.status) {
            case 2: 
                if(this.isSerbian)
                    retValue = "Uspešno završen"
                else
                    retValue = "Successfully finished"
                break
            case 3:
                if(this.isSerbian)
                    retValue = "Otkazan"
                else
                    retValue = "Canceled"
        }   
        return retValue
    },
    role() {
        if(this.isRunner) {
            return this.isSerbian ? 'Pružalac usluge' : 'Runner'
        }
        else
            return this.isSerbian ? 'Potražilac usluge' : 'Request creator'
    },
    picturesTaken() {
        let taken = true
        const len = this.computedRequest.tasklist.length
        const tasks = this.computedRequest.tasklist
        for(let i = 0; i < len; i++) {
            if(tasks[i].picture_required && tasks[i].pictures.length == 0) {
                taken = false
                break
            }
        }
        if(taken && this.computedRequest.picture_required && this.computedRequest.pictures.length == 0)
            taken = false
        return taken
    },
    spanColor() {
        if(this.computedRequest.status == 2)
            return 'green'
        else 
            return 'red'
    },
    isParticipant() {
      if(this.computedRequest.created_by.id == this.$store.getters['getAuthUserId'])
        return true
      if(this.computedRequest.working_with && this.computedRequest.working_with.id == this.$store.getters['getAuthUserId'])
        return true
      return false
    }
  },
  methods: {
    tryToRate() {
      this.showModalAreYouSure = true
    },
    rateUser() {
      this.$emit('setMessages')

      const filters = {
        "rated_user" : this.userToRate.id,
        "request" : this.request.id,
        "grade" : this.rating.grade,
        "comment" : this.rating.comment
      }
      const isInUnratedCreated = this.request.created_by.id != this.userToRate.id ? false : true
      this.$store.dispatch('addRating', {filters, isInUnratedCreated})
      this.showModalAreYouSure = false
      this.$emit('close')
    }
  },
  created() {
      this.$store.state.specificRequest = null
      this.$store.dispatch('getRequestByIdBasic', this.request)
  }
}
</script>

<style scoped>

  .modal-mask-report {
    position: fixed;
    z-index: 9998;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background-color: rgba(0, 0, 0, 0.5);
    display: flex;
    transition: opacity 0.3s ease;
    justify-content: center;
  }

  .big-z-index {
    z-index: 99999999999999999;
  }

  .modal-wrapper {
    display: table-cell;
    vertical-align: middle;
    display: flex;
    padding-top: 80px;
    height: 100%;
    align-items: center;
    justify-content: center;
  }

  .modal-container {
    width: 500px;
    margin: 0px auto;
    padding: 20px 30px;
    background-color: #fff;
    border-radius: 2px;
    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.33);
    transition: all 0.3s ease;
    font-family: Helvetica, Arial, sans-serif;
    max-height: 90%;
    overflow-y: scroll;
  }

  @media only screen and (max-width: 600px)
  {

      .modal-container {
        padding: 5px 10px;
        width:90%;
        min-width: 300px;
      }
  }

  @media only screen and (max-width: 499px)
  {
      .modal-wrapper {
        padding-top: 95px;
      }
  }

  .modal-header{
    margin-top: 0;
    color: rgb(41, 80, 207);
    font-size: 18px;
  }

  .modal-body {
    margin: 10px;
  }


  .modal-default-button {
    float: right;
  }

  .modal-enter {
    opacity: 0;
  }

  .modal-leave-active {
    opacity: 0;
  }

  .modal-enter .modal-container,
  .modal-leave-active .modal-container {
    -webkit-transform: scale(1.1);
    transform: scale(1.1);
  }

  .info-wrap {
      display: flex;
      flex-direction: column;
      align-items: flex-start;
  }

  .info-span {
      margin: 5px 0px 5px 0px;
      display: flex;
      align-items: center;
      flex-wrap: wrap;
  }

  .info-pic {
      height: 28px;
      width: 28px;
  }

  .info-title {
      margin-right: 3px;
  }

  .green {
    color: rgb(2, 97, 2);
  }

  .red {
    color: rgb(235, 16, 16);
  }

  strong {
      margin-right: 5px;
  }

  strong .info-pic-status {
      margin-right: 0px;
  }









  .grade-label {
    width:fit-content;
    font-size: 18px;
  }

  .text-danger {
    margin-top:10px;
    font-size:12px;
  }

  .txt-area {
    margin-top:40px;
  }

  .form-control {
    width:100%;
  }

  .btns {
    display: flex;
    flex-direction: row-reverse;
    margin: 10px 10px 0 0;
  }

  .action-btn {
    margin: 0 0 0 5px;
  }

  .vue-slider {
    margin-top: 30px;
  }

  .select-type {
    width:100%;
    margin-top:5px;
  }

  .is-128x128 {
    width: 70px;
    height: 70px;
    border-radius: 5px;
    margin: 0px 5px 5px 0px;
  }

  .rounded-image {
    border-radius: 5px;
    border: 1px solid black;
    height: 70px;
    width: 70px;
    object-fit:cover;
    position: relative;
    top:0;
    left:0; 
    cursor:pointer
  }

  .no-pointer {
    cursor: auto;
  }

  .transparent {
    position: absolute;
    opacity: 0;
  }

  .semi-transparent {
    position: absolute;
    opacity: 0.5;
  }

  .shown {
    visibility:visible;
  }

  .hidden {
    visibility:hidden;
    height: 0px;
  }

  .picture-msg {
    font-size: 16px;
    width: 100%;
    border: 2px solid transparent;
    border-radius: 5px;
    padding: 2px;
    margin: 0px -5px 0px -5px;
  }

  .picture-msg-dragged {
    border: 2px solid lightgrey;
    cursor: pointer;
    background-color: rgb(235, 230, 230);
    font-size: 16px;
    width: 100%;
    border-radius: 5px;
    padding: 2px;
  }

  .upload-pic-div {
    display: flex;
    flex-direction: column;
    width: fit-content;
    margin-bottom: 5px;
    margin-top: 15px;
  }

  .remove-pic-div {
    width: fit-content;
    align-self: flex-start;
    margin-bottom: 5px;
    display: flex;
    align-items: center;
    font-weight: 600;
  }

  .remove-pic-div:hover {
    cursor: pointer;
    color: blue;
  }

  .remove-pic {
    height:20px;
    width: 20px;
    margin: 2px 0 0 2px;
    top: 0;
    left:0;
    position: absolute;
    border:1px solid transparent;
    border-radius: 20px;
    background-color: red;
  }

  .remove-pic:hover {
    cursor: pointer;
    border: 1px solid grey;
  }

  .media-center {
    display: flex;
    flex-direction: row;
    flex-wrap: wrap;
    margin-bottom: 5px;
  }

</style>